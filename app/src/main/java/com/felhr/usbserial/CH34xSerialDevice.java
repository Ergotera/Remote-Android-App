...
public class CH34xSerialDevice extends UsbSerialDevice
{
    ...
    public void setBaudRate(int baudRate)
    {
        if(baudRate <= 300)
        {
            ...
            if(ret == -1)
                Logger.Info(CLASS_ID, "SetBaudRate failed!");
    ...
    private boolean openCH34X()
    {
        ...
        {
            Logger.Info(CLASS_ID, "Interface succesfully claimed");
        }else
        {
            Logger.Info(CLASS_ID, "Interface could not be claimed");
            return false;
        }
    ...
    private int init()
    {
        ...
        {
            Logger.Info(CLASS_ID, "init failed! #1");
            return -1;
        }

        if(setControlCommandOut(0xa4, 0xdf, 0, null) < 0)
        {
            Logger.Info(CLASS_ID, "init failed! #2");
            return -1;
        }

        if(setControlCommandOut(0xa4, 0x9f, 0, null) < 0)
        {
            Logger.Info(CLASS_ID, "init failed! #3");
            return -1;
        }
        ...
        if(setControlCommandOut(0x9a, 0x2727, 0x0000, null) < 0)
        {
            Logger.Info(CLASS_ID, "init failed! #10");
            return -1;
        }
    ...
    private int checkState(String msg, int request, int value, int[] expected)
    {
        ...
        {
            Logger.Info(CLASS_ID, ("Expected " + expected.length + " bytes, but get " + ret + " [" + msg + "]"));
            return -1;
        }else
        {
            return 0;
    ...
    private boolean checkCTS()
    {
        ...
        if(ret != 2)
        {
            Logger.Info(CLASS_ID, ("Expected " + "2" + " bytes, but get " + ret));
            return false;
        }
    ...
    private int writeHandshakeByte()
    {
        ...
        {
            Logger.Info(CLASS_ID, "Failed to set handshake byte");
            return -1;
        }
        return 0;
    }
    ...
    private int setControlCommandOut(int request, int value, int index, byte[] data)
    {
        ...
        Logger.Info(CLASS_ID,"Control Transfer Response: " + String.valueOf(response));
        return response;
    }
